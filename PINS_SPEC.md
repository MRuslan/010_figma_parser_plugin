...
### 2.1. Главный конфиг (значение для ключа `map_pins`)

Плагин выгружает «голый» объект — без `export default ...;` обёртки и без внешнего ключа `map_pins`. Вставляется прямо как значение в проекте, например `map_pins: <ЗДЕСЬ>` или в любой другой контекст.
...
```js
{
    ...
    minZoom: 2,                      // только если в Figma были zoom-слои; нижняя граница (включительно)
    maxZoom: 2.99,                   // только если zoom-слои; верхняя граница ИСКЛЮЧИТЕЛЬНО (см. логику ниже)
    left, top, isRight, text, breakpoints,
}
```

При наличии language/zoom-слоёв **один логический пин разворачивается в несколько элементов** массива: по одному на каждую комбинацию (lang × zoom). Фронт должен фильтровать массив по текущему `language` и `currentZoom` (попадает ли в диапазон `[minZoom, maxZoom]`).

#### Логика minZoom / maxZoom

Правила формирования диапазона (наследие из схемы landmarks v1):

| Случай | minZoom | maxZoom |
|---|---|---|
| Пин в zoom_1 и больше нигде нет zoom_2 (zoom_1 — единственный уровень) | **опущен** (видим с начала) | **опущен** (видим до конца) |
| Пин в zoom_1, есть zoom_2, и пин ТАМ ЕСТЬ | опущен | опущен (плавный переход на следующий уровень) |
| Пин в zoom_1, есть zoom_2, но пина ТАМ НЕТ | опущен | **1.99** (виден до начала zoom 2) |
| Пин в zoom_2 (последний уровень) | **2** | опущен |
| Пин в zoom_2, есть zoom_3, и пин ТАМ ЕСТЬ | 2 | опущен |
| Пин в zoom_2, есть zoom_3, но пина ТАМ НЕТ | 2 | **2.99** |

> **Важно:** один и тот же пин (одинаковый `text`+`language`), присутствующий в нескольких zoom-слоях, **порождает несколько отдельных записей** в массиве — по одной на каждый zoom. У каждой свой диапазон `minZoom/maxZoom` и потенциально своя позиция (если в Figma пин «переехал» между уровнями зума). Фронт должен показывать ту запись, чей диапазон включает `currentZoom`.

#### Логика отображения на стороне фронта

```ts
function isPinVisibleAt(pin, currentZoom) {
    if (pin.minZoom !== undefined && currentZoom < pin.minZoom) return false;
    if (pin.maxZoom !== undefined && currentZoom > pin.maxZoom) return false;
    return true;
}
```

`maxZoom = N.99` означает «видим до тех пор, пока `currentZoom < N+1`». Если у тебя дробный зум (1.5, 1.8, ...) — это работает естественно. Если зум целочисленный (1, 2, 3) — `currentZoom = 2` пройдёт фильтр `> 1.99` и пин скроется, что и нужно.
...
> Все три конфига плагин выгружает **без** `export default` / завершающего `;` — это «голые» литералы для прямой вставки в твой код. Куда конкретно вставлять (как значение свойства, как присваивание `const`, как импортируемый файл) — решает разработчик.
...
### Дубли текста (несколько пинов с одинаковым названием)

Несколько пинов могут иметь **одинаковый `text`** — например, две заправки «ADNOC» в разных местах, или несколько «Retail» в одном вьюпорте. Это полностью поддерживается:

- В массиве `pins` будет **несколько отдельных записей** с одинаковым `text`, но разными `left`/`top`.
- В i18n такой `text` присутствует **один раз** — все одноимённые пины показывают одну подпись (это намеренно: одинаковый текст = одинаковый перевод).
- **Важно:** не используй `pin.text` как уникальный React-`key` / DOM-`id` — он может повторяться. Используй индекс массива или `text + idx`.

> Как парсер их различает: каждый Pin-инстанс в Figma — отдельная нода, все они сохраняются. Сопоставление desktop ↔ mobile внутри группы идёт по ближайшей позиции иконки, так что одноимённые пины не «слипаются».
...
```tsx
// pinsConfig — объект с группами как ключами (значение, выгруженное парсером).
// Импортируется так, как тебе удобно — например:
//   const pinsConfig = { map_education: {...}, map_health: {...}, ... };
// или из отдельного файла, где этот объект экспортируется тем способом, который принят в проекте.
import { pinsConfig, pinsSvg, pinsI18n } from './pins-config';
...
export function MapPins({ isMobile, lang, mapWidth }: MapPinsProps) {
    ...
    return Object.entries(pinsConfig).map(([groupKey, group]) => {
...
- [ ] Подключить 3 конфига: главный объект пинов, SVG paths, переводы. Парсер выгружает их как «голые» объекты — обернуть в `const` / `export default` / любой другой формат твоего проекта самостоятельно
- [ ] Положить SVG-файлы в `public/svg/map/{name}/pins/`
- [ ] Заменить плейсхолдер `[name_map]` в SVG paths на имя карты
...
- [ ] При наличии `language` в массиве `pins` — фильтровать по текущему языку
- [ ] При наличии `minZoom`/`maxZoom` — фильтровать по текущему `currentZoom` (см. раздел «Логика minZoom / maxZoom»)
