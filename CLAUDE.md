...
src/
  plugin/
    ...
    schemas/
      ...
      streets-detect.ts        — авто-определение варианта структуры Streets (Viewports/Languages)
      streets.ts               — парсер схемы Map Streets (названия улиц на карте)
      districts-detect.ts      — авто-определение варианта структуры Districts (Viewports/Languages/Zooms)
      districts.ts             — парсер схемы Map Districts (районы на карте)
      pins-detect.ts           — авто-определение варианта структуры Pins (Viewports/Languages/Zooms)
      pins.ts                  — парсер схемы Map Pins (POI-пины: retail/health/education/...)
  ui/
...
| `streets` | Map Streets | `streets.ts` |
| `districts` | Map Districts | `districts.ts` |
| `pins` | Map Pins | `pins.ts` |
